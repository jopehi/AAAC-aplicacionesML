...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Reentrena el baseline SSH a partir de un log real o exportado.")
    parser.add_argument("--config", type=Path, default=Path("config/settings.yaml"))
    parser.add_argument("--input-log", type=Path, default=Path("/var/log/auth.log"))
    parser.add_argument("--events-output", type=Path, default=Path("data/processed/ssh_events.csv"))
    parser.add_argument("--features-output", type=Path, default=Path("data/processed/ssh_features.csv"))
    parser.add_argument("--model-output", type=Path, default=Path("models/ssh_anomaly_model.joblib"))
    parser.add_argument("--metadata-output", type=Path, default=Path("models/model_metadata.json"))
    parser.add_argument("--year", type=int, default=2026)
    parser.add_argument("--summary-output", type=Path, default=Path("models/retrain_summary.json"))
    return parser
